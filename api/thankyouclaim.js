...
//
// Notes:
// - This version FAILS FAST on KV/Shopify transport/auth errors instead of mislabeling them
//   as "already claimed". If Upstash/Shopify is down or misconfigured, you’ll get a 500 with
//   a clear message (e.g., kv_unavailable).
// - Debug mode: add ?debug=1 to the POST URL or {debug:true} in JSON body to inspect keys
...
// ----- Upstash helpers (throw on transport/auth errors) -----
...
async function kvSetNxEx(key, value, ttlSeconds) {
  ...
  const data = await r.json().catch(() => null);
  // When NX fails (key exists), Upstash returns { result: null } with 200
...
async function createDiscountBasic({ code, startsAt, endsAt, percent }) {
  ...
  if (!ok) throw new Error(`Shopify HTTP ${status} (check SHOPIFY_SHOP / token / API version ${API_VERSION})`);
...
function setDebugHeaders(res, obj = {}) {
  ...
  res.setHeader('Access-Control-Expose-Headers', [
    ...
    'X-TY-guestMonthKey','X-TY-custMonthKey','X-TY-activeIpKey','X-TY-reason'
  ].join(', '));
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (DEBUG) {
      ...
      const activeOk = existingAct && existingAct.endsAt && Date.parse(existingAct.endsAt) > nowMs;
      ...
      return res.status(200).json({
        ...
        existing: {
          guestMonthly: existingGuest, customerMonthly: existingCust,
          activeIp: existingAct, everIp: existingEverI, everCustomer: existingEverC
        },
    ...
    try {
      if (customerKeyHash && kvKeyCustPeriod) {
        ...
        if (!ok1) {
          const existing = await kvGet(kvKeyCustPeriod); // throws if KV down
        ...
        if (!ok2) {
          await kvDel(kvKeyCustPeriod).catch(()=>{});
      ...
      setDebugHeaders(res, { reason: 'kv-failure', kvError: String(kvErr?.message || kvErr) });
    ...
    if (hasRealIp && kvKeyActiveIp) {
      const existing = await kvGet(kvKeyActiveIp).catch(e => ({ kvError: String(e?.message || e) }));
      if (existing && !existing.kvError) {
    ...
    setDebugHeaders(res, { reason: 'server-error', error: String(e?.message || e), apiVersion: API_VERSION, shop: SHOPIFY_SHOP });
