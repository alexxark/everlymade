...
//
// Works with either Vercel KV or Upstash Redis REST.
// ENV VARS (any of these; first found wins):
...
function kvStyle(url) {
  ...
  // vercel-storage.com → lowercase params (ex=, nx=true)
  if (u.includes('kv.vercel-storage.com')) return 'vercel';
  // upstash.io → uppercase params (EX=, NX=1)
  if (u.includes('.upstash.io')) return 'upstash';
  // default to vercel style (most forgiving)
  return 'vercel';
}
...
// ----- KV helpers (throw on transport/auth errors) -----
...
async function kvSetNxEx(key, value, ttlSeconds) {
  ...
  const data = await r.json().catch(() => null);
  return data?.result === 'OK'; // NX failed → { result: null }
}

async function kvDel(key) {
  if (!KV_URL || !KV_TOKEN || !key) return false;
...
async function kvSet(key, value) {
  if (!KV_URL || !KV_TOKEN) throw new Error('KV not configured');
  const url = `${KV_URL}/set/${encodeURIComponent(key)}/${encodeURIComponent(JSON.stringify(value))}`;
  const r = await fetch(url, { method: 'POST', headers: { Authorization: `Bearer ${KV_TOKEN}` }});
